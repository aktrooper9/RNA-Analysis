...
# Filter females with mgs_level 1 or 4
pdata_clean <- subset(pdata, Sex == "F" & mgs_level %in% c("1", "4") & !is.na(mgs_level))
...
# ------------------------
counts <- read.table("GSE115828_raw_counts_GRCh38.p13_NCBI.tsv", header = TRUE, sep = "\t", row.names = 1, check.names = FALSE)
...
write.csv(results_summary, "results/tables/deg_summary_mgs_1_vs_4_females.csv")
...
png("results/plots/deg_counts_plot_mgs_1_vs_4_females.png", width = 800, height = 600)
ggplot(df_long, aes(x = LFC_Cutoff, y = Count, fill = Category)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "DEG Counts: mgs_level 4 vs 1 (Females Only)",
...
# Save DESeq2 Results
# ------------------------
write.csv(as.data.frame(res_mgs), "results/tables/deseq2_results_mgs_4_vs_1_females.csv")

res_sig <- res_mgs[!is.na(res_mgs$padj) & res_mgs$padj < fdr_threshold & abs(res_mgs$log2FoldChange) > log2(1.5), ]
write.csv(as.data.frame(res_sig), "results/tables/deseq2_sig_results_mgs_1_vs_4_females.csv")
write.csv(norm_counts, "results/tables/normalized_counts_mgs_1_vs_4_females.csv")

# ------------------------
# Plots: MA, Volcano, Top DEGs
# ------------------------
png("results/plots/ma_plot_mgs_1_vs_4_females.png", width = 800, height = 600)
plotMA(res_mgs, ylim = c(-4, 4), main = "MA Plot: mgs_level 4 vs 1 (Females)")
dev.off()
...
volcano_plot <- ggplot(res_df, aes(x = log2FoldChange, y = -log10(padj), color = significant)) +
  ...
  labs(title = "Volcano Plot: mgs_level 4 vs 1 (Females)",
       x = "Log2 Fold Change", y = "-Log10 Adjusted P-value") +
  theme_minimal()

ggsave("results/plots/volcano_plot_mgs_1_vs_4_females.png", volcano_plot, width = 8, height = 6)
saveRDS(volcano_plot, file = "results/rds_plots/volcano_plot_mgs_1_vs_4_females.rds")
...
if(nrow(top_genes) > 0) {
  top_degs_plot <- ggplot(top_genes, aes(x = reorder(rownames(top_genes), log2FoldChange),
    ...
    coord_flip() +
    labs(title = "Top 10 DEGs: mgs_level 4 vs 1 (Females)",
         x = "Gene", y = "Log2 Fold Change") +
    scale_fill_manual(values = c("TRUE" = "red", "FALSE" = "blue")) +
    theme_minimal()
  ggsave("results/plots/top_degs_plot_mgs_1_vs_4_females.png", top_degs_plot, width = 8, height = 6)
  saveRDS(top_degs_plot, file = "results/rds_plots/top_degs_plot_mgs_1_vs_4_females.rds")
}
...
print(go_dot)
dev.off()
saveRDS(go_dot, file = "results/rds_plots/go_bp_dotplot_mgs_1_vs_4_females.rds")
...
print(kegg_dot)
dev.off()
saveRDS(kegg_dot, file = "results/rds_plots/kegg_dotplot_mgs_1_vs_4_females.rds")
...
print(react_bar)
dev.off()
saveRDS(react_bar, file = "results/rds_plots/reactome_barplot_mgs_1_vs_4_females.rds")
...
if(length(top_genes_heatmap) > 0) {
  ...
  pheatmap(top_counts_sorted,
           ...
           main = paste("Heatmap of Top", length(top_genes_heatmap), "DEGs: mgs_level 4 vs 1 (Females)"),
           filename = "results/plots/heatmap_top_DEGs_mgs_1_vs_4_females.png")
...
umap_plot <- ggplot(umap_data, aes(x = V1, y = V2, color = mgs_level)) +
  geom_point(size = 3) +
  labs(title = "UMAP of Normalized Counts: mgs_level Groups (Females Only)",
       x = "UMAP1", y = "UMAP2") +
  theme_minimal() +
  scale_color_manual(values = c("1" = "blue", "4" = "red"))

ggsave("results/plots/umap_plot_mgs_1_vs_4_females.png", umap_plot, width = 8, height = 6)
saveRDS(umap_plot, file = "results/rds_plots/umap_plot_mgs_1_vs_4_females.rds")

# ------------------------
# Session Info and Final Messages
# ------------------------
writeLines(capture.output(sessionInfo()), "results/session_info_mgs_1_vs_4_females.txt")
